...
Additional mounts can be specified as part of your [cloud-config](/configuration/#cloud-config). These mounts are applied within the console container. Here's a simple example that mounts `/dev/vdb` to `/mnt/s`.
...
**hint**: You need to pre-format the disks, burmilla-os will not do this for you. The mount will not work (silently) until you formatted the disk, e.g. using:
...
With burmilla 1.1.1+ you do no longer need to create the mount-point folder, it will be created automatically.
